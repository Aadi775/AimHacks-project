  ...
  return (
    <section className="max-w-[1360px] mx-auto px-gutter-mobile md:px-gutter xl:px-margin pt-8 md:pt-14 pb-12 flex flex-col items-center text-center">
      ...
      <div className="inline-flex items-center gap-space-xs px-3.5 py-1.5 rounded-full bg-surface-container-lowest/90 backdrop-blur-md shadow-sm mb-6">
        ...
        <span className="font-label-xs text-label-xs text-on-surface-variant uppercase tracking-wider">Jaipur Telemetry Pulse</span>
        <span className="text-outline-variant font-label-xs text-label-xs">/</span>
        <span className="font-label-xs text-label-xs text-on-tertiary-container font-medium">Optimal Conditions</span>
      </div>
      {/* Hero Heading */}
      ...
      <p className="font-body-lg text-body-lg text-on-surface-variant max-w-2xl mt-4 leading-relaxed font-normal">
        Hyperlocal environmental vitals, seamless transit telemetry, and verified neighborhood dispatches across Jaipur.
      </p>
      {/* Search Bar */}
      ...
      <div className="w-full max-w-4xl mt-10 bg-surface-container-lowest/80 backdrop-blur-md rounded-2xl p-4 shadow-sm">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-left">
          {[
            { icon: 'verified', label: 'Civic Index', value: '88', sub: 'Optimal', color: 'on-tertiary-container' },
            { icon: 'air', label: 'Air Quality', value: '24', sub: 'AQI Crisp', color: 'secondary' },
            { icon: 'tram', label: 'Transit Cadence', value: '94%', sub: 'On-Time', color: 'on-surface-variant' },
            { icon: 'bolt', label: 'Clean Grid', value: '100%', sub: 'Hydro/Solar', color: 'on-tertiary-container' },
          ].map((m) => (
            <div key={m.label} className="flex items-center gap-3 p-2 rounded-xl bg-surface-container-low/50">
              <div className={`w-8 h-8 rounded-full bg-surface-container-high flex items-center justify-center text-${m.color}`}>
              ...
              <div className="min-w-0">
                ...
                <span className="block font-headline-sm text-headline-sm text-on-surface font-semibold">{m.value} <span className={`font-label-xs text-label-xs text-${m.color} font-medium`}>{m.sub}</span></span>
              </div>
            </div>
          ))}
        </div>
